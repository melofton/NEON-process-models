...
team_name <- "procEppleyNorbergMonod"
...
proc_model <- function(par, wtemp, chla, swr){
  ...
  for(i in 2:length(wtemp)){
    ...
    fT = eppley_norberg(wtemp = wtemp[i],
              z = par[1],
              w = par[2],
              a = par[3],
              b = par[4])
  ...
  par <- c(-237.4142225,	270.9646882,	0.054341024,	0.218400435, 200, 0.2, 0.2)
...
#                     w = rep(NA, length(sites)),
#                     a = rep(NA, length(sites)),
#                     b = rep(NA, length(sites)),
#                     I_K = rep(NA, length(sites)),
#                     R_growth = rep(NA, length(sites)),
#                     R_resp = rep(NA, length(sites)),
#                     RMSE = rep(NA, length(sites)))
parms <- read.csv("./Models/procEppleyNorbergMonodParameters.csv")
parms[j,c(2:9)] <- c(fit2$par,rmse(par = fit2$par, chla, wtemp, swr))
write.csv(parms, "./Models/procEppleyNorbergMonodParameters.csv", row.names = FALSE)
...
p5 <- read.csv("./Models/procHinshelwoodMonodParameters.csv")
p6 <- read.csv("./Models/procHinshelwoodSteeleParameters.csv")
p7 <- read.csv("./Models/procEppleyNorbergSteeleParameters.csv")

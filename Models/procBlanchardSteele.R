...
parms <- read_csv("./Models/procBlanchardSteeleParameters.csv")
...
for(j in 1:nrow(forecast_starts)){
  ...
  par = parms %>%
    ...
    pivot_longer(Topt:R_resp, names_to = "parameter", values_to = "value") %>%
    pull(value)
...
procBlanchardSteele_EFI <- convert.to.efi_standard(forecast)
  

forecast_file <- paste0('aquatics-', procBlanchardSteele_EFI$reference_datetime[1], '-', team_name, '.csv.gz')

write_csv(procBlanchardSteele_EFI, forecast_file)
# Submit forecast!

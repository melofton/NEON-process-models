...
proc_model <- function(par, wtemp, chla, swr){
  ...
  par <- c(30, 35, 0.4, 0.01, 700, 0.2, 0.2)
...
parms <- read.csv("./Models/procBlanchardSteeleParameters.csv")
parms[j,c(2:9)] <- c(fit2$par,rmse(par = fit2$par, chla, wtemp, swr))
write.csv(parms, "./Models/procBlanchardSteeleParameters.csv", row.names = FALSE)

...
team_name <- "procEppleyNorbergSteele"
...
proc_model <- function(par, wtemp, chla, swr){
  ...
  par <- c(-237.4142225,	270.9646882,	0.054341024,	0.218400435, 250, 0.4, 0.4)
...
#                     I_S = rep(NA, length(sites)),
#                     R_growth = rep(NA, length(sites)),
#                     R_resp = rep(NA, length(sites)),
#                     RMSE = rep(NA, length(sites)))
parms <- read.csv("./Models/procEppleyNorbergSteeleParameters.csv")
parms[j,c(2:9)] <- c(fit2$par,rmse(par = fit2$par, chla, wtemp, swr))
write.csv(parms, "./Models/procEppleyNorbergSteeleParameters.csv", row.names = FALSE)

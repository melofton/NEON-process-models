...
message('starting procEppleyNorbergSteele forecast generations')
...
parms <- read_csv("./Models/procEppleyNorbergSteeleParameters.csv")
...
for(j in 1:nrow(forecast_starts)){
  ...
  par = parms %>%
    ...
    pivot_longer(z:R_resp, names_to = "parameter", values_to = "value") %>%
    pull(value)
...
procEppleyNorbergSteele_EFI <- convert.to.efi_standard(forecast)
  

forecast_file <- paste0('aquatics-', procEppleyNorbergSteele_EFI$reference_datetime[1], '-', team_name, '.csv.gz')

write_csv(procEppleyNorbergSteele_EFI, forecast_file)
# Submit forecast!
